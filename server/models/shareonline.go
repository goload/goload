...
func (so *ShareOnline) getDirectLink(file *File) (string, error) {
	...
	for _, line := range strings.Split(strings.TrimSuffix(stringBody, "\n"), "\n") {
		...
		if split[0] == "URL" {
			url = strings.ReplaceAll(strings.TrimSpace(line[4:]), "http://", "https://")
		}
		if split[0] == "MD5" {
			if file.Checksum != strings.TrimSpace(line[4:]) {
				log.Println("WARNING: Checksum mismatch")
			}
			file.Checksum = strings.TrimSpace(line[4:])
		}
	}
	if url == "" {
		return "", err
